        ...
        <div className="flex flex-col p-2">
          {data.experiences.map((item) => (
            <div key={item.id} className="w-full h-auto flex- flex-col ">
              <div className="w-full h-auto flex flex-col lg:flex-row lg:justify-between gap-7 items-center relative">
                <div
                  className={` w-full lg:w-[60%]  relative  border-amber-300  p-6  leading-8 text-justify  text-zinc-700  dark:text-amber-50  ${lang === "en" ? "border-l" : "border-r"} `}
                >
                  {/* Timeline Dot */}
                  <span
                  ...
                  <span className="mb-2 block text-lg font-semibold text-amber-300">
                    {item.company[lang]} . {item.duration[lang]}
                  </span>
                  {/* Timeline Status*/}
                  <div
